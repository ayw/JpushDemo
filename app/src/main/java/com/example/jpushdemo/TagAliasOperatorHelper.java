...
public class TagAliasOperatorHelper {
    ...
    public void checkIfNeedRetryAdd(Context context, Set<String> tags) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!ExampleUtil.isValidTagAndAlias(tag)) {
                Log.e(TAG, "checkIfNeedRetry: 标签不合法！");
                continue;
            }
            int recordFlag = SPUtils.getInstance(TAGSP).getInt(tag, NUll);
            String msg = "";
            switch (recordFlag) {
                case ADD_SUC:
                    msg = "ADD_SUC";
            ...
            Log.i(TAG, "checkIfNeedRetryAdd:  SP取出的消息记录\n ————> " + tag + " : " + msg);
            switch (recordFlag) {
                //没有绑定Tag操作的记录，就去新增TAG
                case NUll:
                    tagAliasBean.action = ACTION_ADD;
                    tagAliasBean.tags.add(tag);
                    break;
                case ADD_SUC:
        ...
        if (jPushMessage.getErrorCode() == 0) {
            ...
            switch (tagAliasBean.action) {
                case ACTION_ADD: {
                    ...
                    while (iterator.hasNext()) {
                        String tag = (String) iterator.next();
                        SPUtils.getInstance(TAGSP).put(tag, ADD_SUC);
                    }
                }
                ...
                case ACTION_DELETE:
                    ...
                    while (iterator.hasNext()) {
                        String tag = (String) iterator.next();
                        SPUtils.getInstance(TAGSP).put(tag, DEL_SUC);
                    }
                    break;
                default:
                    break;
            ...
            switch (tagAliasBean.action) {
                case ACTION_ADD: {
                    ...
                    while (iterator.hasNext()) {
                        String tag = (String) iterator.next();
                        SPUtils.getInstance(TAGSP).put(tag, ADD_FAI);
                    }
                }
                ...
                case ACTION_DELETE: {
                    ...
                    while (iterator.hasNext()) {
                        String tag = (String) iterator.next();
                        SPUtils.getInstance(TAGSP).put(tag, DEL_FAI);
                    }
                }
                break;
                default:
